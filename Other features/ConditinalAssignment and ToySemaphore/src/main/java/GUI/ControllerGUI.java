...
public class ControllerGUI implements Initializable {
    ...
    @FXML
    TableView<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>> semaphoreTable;

    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>, String> semaphoreTableIndex;

    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>, String> semaphoreTableValue1;

    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>, String> semaphoreTableListOfValues;

    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>, String> semaphoreTableValue2;
...
//            System.out.println(program.getSymbolsTable().getContent().entrySet());
    ...
    public void setSemaphoreTable() {
        ...
        ObservableList<HashMap.Entry<Integer, Pair<Integer,Pair<ArrayList<Integer>, Integer>>>> semaphoreTableList = FXCollections.observableArrayList();
        semaphoreTableIndex.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getKey())));
        semaphoreTableValue1.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getKey().toString()));
        semaphoreTableListOfValues.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getValue().toString()));
        semaphoreTableValue2.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getKey().toString()));
        semaphoreTableList.addAll(repository.getProgramStates().get(0).getSemaphoreTable().getSemaphoreTable().entrySet());
